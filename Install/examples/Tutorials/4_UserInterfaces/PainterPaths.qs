...
RenderArea.prototype.paintEvent = function()
{
	...
	painter.translate(-50.0, -50.0);
	painter.setPen(new QPen(this.penColor, this.penWidth, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin));
...
function ScriptCleanupFunction()//Cleanup
{
	try
	{
		win.fillRuleComboBox["activated(int)"].disconnect(win, win.fillRuleChanged);
		win.fillColor1ComboBox["activated(int)"].disconnect(win, win.fillGradientChanged);
		win.fillColor2ComboBox["activated(int)"].disconnect(win, win.fillGradientChanged);
		win.penColorComboBox["activated(int)"].disconnect(win, win.penColorChanged);
		for (var i = 0; i < Window.NumRenderAreas; ++i) 
		{
			win.penWidthSpinBox["valueChanged(int)"].disconnect(win.renderAreas[i], win.renderAreas[i].setPenWidth);
			win.rotationAngleSpinBox["valueChanged(int)"].disconnect(win.renderAreas[i], win.renderAreas[i].setRotationAngle);
		}
	}	
	catch (e) 
	{
	...
	RenderArea.prototype.setFillRule = null;
	RenderArea.prototype.setFillGradient = null;
	RenderArea.prototype.setPenWidth = null;
	RenderArea.prototype.setPenColor = null;
	RenderArea.prototype.setRotationAngle = null;
	RenderArea.prototype.paintEvent = null;
	RenderArea.prototype = null;
	RenderArea = null;
	Window.NumRenderAreas = null;
	Window.prototype.fillRuleChanged = null;
	Window.prototype.fillGradientChanged = null;
	Window.prototype.penColorChanged = null;
	Window.prototype.populateWithColors = null;
	Window.prototype.currentItemData = null;
...
try
{
	win.fillRuleComboBox["activated(int)"].connect(win, win.fillRuleChanged);
	win.fillColor1ComboBox["activated(int)"].connect(win, win.fillGradientChanged);
	win.fillColor2ComboBox["activated(int)"].connect(win, win.fillGradientChanged);
	win.penColorComboBox["activated(int)"].connect(win, win.penColorChanged);
	for (var i = 0; i < Window.NumRenderAreas; ++i) 
	{
		win.penWidthSpinBox["valueChanged(int)"].connect(win.renderAreas[i], win.renderAreas[i].setPenWidth);
		win.rotationAngleSpinBox["valueChanged(int)"].connect(win.renderAreas[i], win.renderAreas[i].setRotationAngle);
	}
}	
catch (e) 
{
