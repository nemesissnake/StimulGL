...
HIDCaptureThread::HIDCaptureThread(unsigned short productID,unsigned short vendorID,QObject *parent) : QThread(parent)
{
	...
	m_cButtonMask = 255;
	m_ButtonDetectDetectionMethod = MaskedValueChanged;
	m_bIsRunning = false;
...
void HIDCaptureThread::ConfigureHIDTriggers(bool bActivateJoystickTrigger, bool bActivateButtonTriggers, unsigned char cButtonMask, DetectionMethod ButtonDetectDetectionMethod)
{
	...
	m_ButtonDetectDetectionMethod = ButtonDetectDetectionMethod;
}
	...
	while(m_bAbortRunning==false)
	{
		...
		if (res>1)
		{
			...
			if(m_bActivateButtonTriggers)
			{
				...
				switch (m_ButtonDetectDetectionMethod)
				{
