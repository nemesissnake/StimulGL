...
var DatFileConcat_currentScriptLocation = StimulGL.getSelectedScriptFileLocation();
	...
	if (searchDatDirectory.length != 0)
	{
		...
		dirDirectory.setPath(searchDatDirectory);
		 if (dirDirectory.exists())
		 {
			...
			dirDirectory.setNameFilters(new Array("*.dat"));
...
{
	DatFileConcat_searchDatDirectory = "E:/Projects/StimulGL/Install/outputs/RetinoWidget/20130508143814431_Retinotopic Mapping Experiment with the use of a TriggerTimer or ParallelPort/";
