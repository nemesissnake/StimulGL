...
function CustomScriptHandlerFunction(pixMap)
{
	//Log("--- CustomScriptHandlerFunction arguments count: " + arguments.length);
	//for (var i = 0; i < arguments.length; ++i)
	//	Log("\t argument(" + i + "):" + arguments[i]); 
	if(pixMap == "QPixmap")
	{
...
function ExperimentStateChanged()
{
	...
	{	
		...
		RetinoMap_RenderWidgetGL.installCustomScriptHandlerFunction("CustomScriptHandlerFunction");	
	}	
	if(arg == 8)
	{
